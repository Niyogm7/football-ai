...
def to_number(value, default=0):
    """Convert SofaScore stat values to int (remove % if needed)."""
    try:
...
if matches:
    ...
    if match:
        ...
        # --- Safe numeric values ---
        home_shots = to_number(sofascore_stats.get("Shots on target", {}).get("home", 5), 5)
        away_shots = to_number(sofascore_stats.get("Shots on target", {}).get("away", 5), 5)
        home_poss = to_number(sofascore_stats.get("Ball possession", {}).get("home", 50), 50)
        away_poss = to_number(sofascore_stats.get("Ball possession", {}).get("away", 50), 50)
        home_cards = to_number(sofascore_stats.get("Yellow cards", {}).get("home", 1), 1)
        away_cards = to_number(sofascore_stats.get("Yellow cards", {}).get("away", 1), 1)
